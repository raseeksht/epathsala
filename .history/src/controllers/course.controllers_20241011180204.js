...
const deleteCourse = asynchHandler(async (req, res) => {
    const _id = req.params._id;
    if (!await courseModel.findOne({ _id })) {
        throw new ApiError(404, "Course does not exits.")
    }
    const del = await courseModel.deleteOne({ _id });
    if (del.deletedCount == 1) {
        res.json(new ApiResponse(204, "deleted successfully", del));
    } else {
        throw new ApiError(400, `Failed while deleting course: ${_id} OR it does not exits!`);
    }
})
