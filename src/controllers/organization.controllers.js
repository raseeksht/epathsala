...
const createOrganization = asyncHandler(async (req, res) => {
    ...
    try {
        const newOrg = await organizationModel.create({ name, description, admin: req.user._id });
        if (newOrg) {
            res.status(201).json(new ApiResponse(201, "New Organization Created", newOrg));
        } else {
            console.log(newOrg)
...
const editOrganization = asyncHandler(async (req, res) => {
    ...
    const org = await organizationModel.findOne({ _id: organizationId }).populate([{
        path: "admin",
...
const getOrganization = asyncHandler(async (req, res) => {
    ...
    const org = await organizationModel.findOne({ _id: orgId }).populate("admin", "username email");
